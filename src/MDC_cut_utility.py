...
import psutil
import zarr
from MDC_cut import loadfiles, fit_so_app, copy_to_clipboard
...
class app_param:
    def __init__(self, hwnd=None, ScaleFactor=None, scaled_font_size=None, scale=None):
        self.hwnd = hwnd
...
class wait(tk.Toplevel):
    def __init__(self, master, app_pars):
        self.g = master
        self.app_pars = app_pars
        super().__init__(master, background='white')
...
class VolumeSlicer(tk.Frame):
    def __init__(self, parent=None, path=None, volume=np.empty((5,5,5), dtype=np.float32), cmap='gray', x=None, y=None, z=None, ev=None, e_photon=21.2, density=600, g=None, app_pars=None):
        '''
        Args
        ------
        ...
        if x is not None and y is not None:
            ...
            if parent is not None:
                ...
                self.fit_so_button = tk.Button(frame2, text="Fit Sample Offsets", command=fit_so_app, font=('Arial', self.size(14), "bold"), bg='white')
    ...
    def change_mode(self, mode='normal'):
        ...
        if self.type == 'real':
            try:
                ...
                tx = self.app_pars.t_sc_w if self.g.winfo_x()+self.g.winfo_width()/2 > self.app_pars.t_sc_w else 0
                self.g.geometry(f'{w}x{h}+{tx}+{self.app_pars.sc_y}')
                self.g.update()
            except:
                self.type = 'real'
...
class CEC(loadfiles, CEC_Object):
    def __init__(self, g: tk.Tk, files: list[str] | tuple[str, ...] | str, mode: Literal['normal', 'load']= 'normal', cmap: str='viridis', app_pars: Any=None):
    ...
    def __set_data(self, odata=[], density=800, *args):
        if len(odata) > 0:
            ...
            self.tlg.geometry(f'{w}x{h}+{tx}+{self.app_pars.sc_y}')
